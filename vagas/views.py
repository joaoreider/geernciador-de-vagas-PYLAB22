...
def nova_tarefa(request, id_vaga):
    ...
    #TODO: realizar validações do form
...
def envia_email(request, id_vaga):
    ...
    text_cont = strip_tags(html_content)
    email = EmailMultiAlternatives(assunto, text_cont, settings.EMAIL_HOST_USER, [vaga.email, ])
    ...
    if email.send():
        ...
        mail.save()
        messages.add_message(request, constants.ERROR, "Erro ao enviar email")
        return redirect(f'/vagas/vaga/{id_vaga}')